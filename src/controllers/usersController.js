...
exports.users_list = async function(req, res) {
  ...
  try {
    ...
    res.render('users', { title: '管理员用户', navigate: navigate, message: '错误', users: null, error: '返回用户列表时出错：' + err });
  }
}
...
exports.password_post = async function(req, res) {
  ...
  req.checkBody('username', '用户名（必填）').notEmpty();
  req.sanitize('username').escape();
  req.sanitize('username').trim();

  req.checkBody('password1', '需要密码').notEmpty();
  req.checkBody('password1', '密码最短长度为 ' + min_pass_len + ' characters').isLength({ min: min_pass_len, max: 160 });

  req.checkBody('password2', '请重新输入密码').notEmpty();
  req.checkBody('password2', '密码最短长度为 ' + min_pass_len + ' characters').isLength({ min: min_pass_len, max: 160 });
  req.checkBody('password2', '密码并不相同').equals(req.body.password1);
